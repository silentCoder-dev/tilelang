"""Cache adapter for CTypes backend."""
...
@CacheAdapterRegistry.register("ctypes")
class CTypesCacheAdapter(BaseCacheAdapter):
    """Cache adapter for CTypes execution backend."""

    def get_cache_files(self, kernel: JITKernel) -> dict[str, CacheFileInfo]:
        """Get files to cache for CTypes kernel.

        CTypes backend caches:
    ...
    def load_from_cache(self, cache_path: str) -> dict[str, Any] | None:
        """Load CTypes kernel data from cache.
    ...
    def get_required_files(self) -> list[str]:
        """Get required files for CTypes cache.
